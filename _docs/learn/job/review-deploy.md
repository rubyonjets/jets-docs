...
The Lambda function is a handler shim generated by Jets as a part of the deployment. It provides the interface required by a Lambda function. The `Jets.process` ultimately calls the `CoolEvent#process` method. You can open `app/jobs/cool_event.rb` and see the code we created earlier.
