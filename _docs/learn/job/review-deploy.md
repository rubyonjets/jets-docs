...
If you look at the CloudFormation stack `demo-dev-CoolEvent-*` Resources tab, you'll see the AWS resources Jets created for `CoolEvent`.
...
Next, let's check the AWS Lambda Console. You can click on the Physical ID link of the `demo-dev-cool_event-handle` to take you there. You'll see something like this:

![](https://img.boltops.com/tools/jets/learn/job/review-deploy-lambda-handler.png)

The Lambda function is a handler shim generated by Jets as a part of the deployment. It provides the interface required by a Lambda function. The `Jets.process` ultimately calls the `CoolEvent#process` method. You can open `app/events/cool_event.rb` and see the code we created earlier.
