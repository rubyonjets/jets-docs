...
app/events  | Contains job definitions. This code usually executes on a scheduled basis asynchronously outside of the request-response cycle.  For more info refer to the [jobs docs]({% link _docs/events.md %})
app/models  | Contains model definitions, usually classes that interact with a database via ActiveRecord or some other ORM.
shared  | Contains shared resources. Refer to [Shared Resources]({% link _docs/custom/shared-resources.md %}).
