---
title: Rails Support with Container Images
nav_text: Rails
category: rails
order: 1
---

Jets supports deploying Rails applications by using Container images. This is a [Jets Pro] feature.
...
Though it is possible to leverage Jets features in Afterburner by adding code to [.jets/project](https://community.rubyonjets.com/t/how-to-test-an-afterburner-job-from-lambda-test-events/203), it's recommended to run Jets natively when possible. When you run a Jets application natively, you also get direct access to the full power of Jets. Some examples are [Jobs]({% link _docs/events.md %}), [Events]({% link _docs/events.md %}), and [IAM Policies]({% link _docs/iam-policies.md %}).
